...
def main():
    ...
    if pamd_in_use():
        ...
        # See which library directories exists
        for pam_dir in pam_spec_args['PAM_MOD_DIRS']:
            if not os.path.exists(pam_dir):
                pam_spec_args['PAM_MOD_DIRS'].remove(pam_dir)

        # Also look in /etc/pam.d for include directives
        pam_spec_args['PAM_MOD_DIRS'] += ['/etc/pam.d']

        # module.fail_json(msg="%s" % pam_spec_args['PAM_MOD_DIRS'])
    else:
